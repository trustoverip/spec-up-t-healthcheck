...
/**
 * Generates two separate rows for console-messages check:
 * 1. Fail row with errors
 * 2. Pass row with all other messages
 * 
 * @param {import('./health-checker.js').HealthCheckResult} result - Console messages result
 * @returns {string} HTML string with two table rows
 */
function generateConsoleMessagesRows(result) {
  const { analysis, errors, allMessages } = result.details;
  ...
  // Row 2: All messages (pass)
  if (allMessages && allMessages.length > 0) {
    // Filter out error messages AND exclude separator, process, and info types
    ...
    const filteredDetails = {
      ...
      analysis: {
        ...
        warningCount: valuableMessages.filter(m => m.type === 'warn').length,
    ...
    html += `<tr data-status="pass" class="check-row table-success">
      ...
      <td>console-messages (operations)</td>
      <td>

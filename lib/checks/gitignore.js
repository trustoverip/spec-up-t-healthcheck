...
/**
 * Required entries that must be present in a valid .gitignore file.
...
export async function checkGitignore(provider) {
  try {
    ...
    const details = {
      ...
      requiredEntriesCount: REQUIRED_ENTRIES.length,
      presentEntriesCount: REQUIRED_ENTRIES.length - missingEntries.length,
